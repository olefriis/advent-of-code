...
fn main() -> std::io::Result<()> {
  let program: Vec<i64> = std::fs::read_to_string("13/input").unwrap()
    .split(',')
  ...
  let blank_input = vec![];
  loop {
    let x = match intcode.run(&blank_input) {
      None => break,
      Some(value) => value,
    };
    let y = intcode.run(&blank_input).unwrap();
    let tile_id = intcode.run(&blank_input).unwrap();

    screen.insert(Position { x: x, y: y }, tile_id);
  }

  let number_of_block_tiles = screen.iter().filter(|(_, &tile_id)| tile_id == 2).count();

  println!("{} block tiles", number_of_block_tiles);
